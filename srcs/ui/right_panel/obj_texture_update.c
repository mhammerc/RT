...
/*   Updated: 2017/04/23 16:20:36 by lmarques         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
															...
															GtkWidget *props)
{
	...
	texture_chooser = gtk_file_chooser_button_new("Texture images",
												GTK_FILE_CHOOSER_ACTION_OPEN);
	if (focused_obj->texture_filename)
		gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(texture_chooser),
												focused_obj->texture_filename);
	g_signal_connect(texture_chooser, "file-set",
											G_CALLBACK(texture_file_set), ui);
...
void		add_texture_type_widgets(t_ui *ui, t_object *focused_obj,
															GtkWidget *props)
{
	...
	gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(texture_type), 0,
															"Planar damier");
	gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(texture_type), 0, "Perlin");
