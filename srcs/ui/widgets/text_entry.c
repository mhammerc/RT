...
GtkWidget		*create_text_entry(gchar *name, gchar *default_value)
{
	...
	g_signal_connect(GTK_ENTRY_BUFFER(text_buffer), "inserted-text", G_CALLBACK(inserted_text), (gpointer)hbox);
	g_signal_connect(GTK_ENTRY_BUFFER(text_buffer), "deleted-text", G_CALLBACK(deleted_text), (gpointer)hbox);
	return (hbox);
}
