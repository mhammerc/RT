...
t_vec3		rand_sphere(t_vec3 c, double r)
{
	t_vec3	v;
	double	dblr;

	dblr = 2.0 * r;
	v.x = c.x + (double)rand() * dblr * INVRANDMAX - r;
	v.y = c.y + (double)rand() * dblr * INVRANDMAX - r;
	v.z = c.z + (double)rand() * dblr * INVRANDMAX - r;
	return (v);
}
...
t_vec3		rt_shadow(t_list *l_obj, t_spot *spot, t_ray ray)
{
	...
	while (i--)
	{
		aim = rand_sphere(spot->pos, spot->radius);
