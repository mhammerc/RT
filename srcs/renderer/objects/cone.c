...
/*   Updated: 2017/03/22 15:03:40 by racousin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				cone_length(t_obj *self, t_ray *ray, t_interval *interval)
{
	double		m;

	if (self->length != 0.)
	{
		m = vec3_dot(ray->dir, vec3_mult(interval->min[0].dist, self->dir))
			+ vec3_dot(vec3_sub(ray->pos, self->pos), self->dir);
		if (m > self->length || m < 0)
		{
			m = vec3_dot(ray->dir, vec3_mult(interval->max[0].dist, self->dir))
				+ vec3_dot(vec3_sub(ray->pos, self->pos), self->dir);
			if (m > self->length || m < 0)
			{
				interval->nb_hit = 0;
...
int				cone_intersect(t_obj *self, t_ray *ray, t_interval *interval)
{
	...
	{
		if (cone_length(self, ray, interval))
			return (0);
