...
t_vec3	global_illum(t_scene *sce, t_ray ray, int depth)
{
	...
	while (--i)
	{
		dir = rand_half_sphere(ray.pos, 1, ray.collided->normal, ray.seed);
		ray.type = REFLECTION_RAY;
		new_light = ray_trace(sce, ray_new_dir(ray, vec3_get_normalized(dir)), depth + 1);
